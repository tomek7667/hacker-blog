...
<!-- <ins>**Recommended: [Full video write-up](TODO)**</ins> -->
...
by calling javascript `.replace` on the `{filename}` which is sanitized previously by the dompurify:
...
A simple showcase of them in use, can help easily understand how does it work in practice. Here we are replacing the `B` character in two cases, where it has stuff before and after it.
...
As you can see, the values before/after were appended in the plase where the `B` was found.

Knowing that such mechanism in JavaScript exists, we can start to try and bypass DOM Purify sanitization, by including _something_ that is not evil payload, but some widely accepted pure string, like `src` of an img. We can test whether it's accepted with a simple node js script importing the `isomorphic-dompurify`, sanitizing the payload and outputing the sanitized one:
...
console.log(cleanPayload); // <img src="http://something">
```
...
To create your own webhook, you can use a simple https server with CORS being enabled, having GET respond with the above html, and the POST saving the data on your disk.
...
The whole payload can be constructed and executed by `eval(atob(<base>))`'ing our javascript code, which can be easily and reproducibly be done by having another javascript create it. The code executed just fetches the `img.png` _(this is the rendered markdown)_ and pushes it to our webhook.
